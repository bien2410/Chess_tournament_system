...
<%
	...
	session.removeAttribute("ttBangXepCap");
	if(nd == null){
		...
		if(request.getParameter("index") == null){
			...
			String apiUrl = "http://localhost:8080/lenLich/giaiDau";
...
%>
<body>
<header><h1>Lên lịch thi đấu </h1></header>
<form method="post" action="GDLenLichThiDau.jsp">
<table>
	<tr>
	<td>Tên giải đấu: </td>
...
<%
		for(int i = 0; i < dsGiaiDau.size(); i++){	
			...
			%>
			<tr onclick="openPage('GDLenLichThiDau.jsp?index=<%=i%>')" style="cursor: pointer;">
				<td><%=i+1%></td>
			...
			try{
				...
				response.sendRedirect("GDXepCapDau.jsp");
			}
			catch(Exception e){
				response.sendRedirect("../GDDangNhap.jsp?err=timeout");
			}
