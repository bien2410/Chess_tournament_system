...
<%
	...
	if(nd == null){
		...
		if(request.getParameter("index") == null){
			...
			String apiUrl = "http://localhost:8080/BXH/giaiDau";
...
%>
<body>
<header><h1>BXH sau từng vòng đấu</h1></header>
<form method="post" action="GDBXHSauTungVongDau.jsp">
<table>
	<tr>
	<td>Tên giải đấu: </td>
...
<%
		for(int i = 0; i < dsGiaiDau.size(); i++){	
			...
			%>
			<tr onclick="openPage('GDBXHSauTungVongDau.jsp?index=<%=i%>')" style="cursor: pointer;">
				<td><%=i+1%></td>
			...
			try{
				...
				response.sendRedirect("GDChonVongDau.jsp");
			}
			catch(Exception e){
				response.sendRedirect("../GDDangNhap.jsp?err=timeout");
			}
...
<button onclick="openPage('GDChonBXH.jsp')" class="back-button">Quay lại</button>
</body>
</html>